import logging
from typing import Dict
from processing.helpers import Context
from processing.fetching import fetch_users

logger = logging.getLogger(__name__)

USER_SET_KEY = 'filtered-users-set'

def filter_out(context: Context, event: Dict) -> bool:
    username = event.get('initiator_user_name')
    filtered_out = _filter_out_users(context, username)
    return filtered_out


def _filter_out_users(context: Context, username: str):
    if username:
        if not context.store.exists(USER_SET_KEY):
            usernames = fetch_users(context.gsheets, context.config)
            if usernames:
                context.store.set_set(USER_SET_KEY, usernames)
            else:
                logger.error('User list from google spreadsheet is empty')
        if context.store.is_in_set(USER_SET_KEY, username):
            return True
    return False

